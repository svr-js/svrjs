...
module.exports = (req, res, logFacilities, config, next) => {
  ...
  const properDirectoryListingAndStaticFileServe = (fd, stats) => {
    if (stats.isFile()) {
      ...
      if (req.headers["range"]) {
          ...
          if (!regexmatch) {
            ...
            if (req.method != "HEAD") {
              if (
                ...
                return;
              } else if (ext == "html" && begin >= res.head.length + filelen) {
                res.writeHead(206, statusCodes[206], rhd);
                res.end(
                  res.foot.substring(
              ...
              let readStream = fs.createReadStream(null, {
                start:
                  ext == "html" ? Math.max(0, begin - res.head.length) : begin,
                end:
                  ext == "html"
                ...
                if (ext == "html") {
                  const afterWriteCallback = () => {
                    ...
                    readStream.pipe(res, {
                        ...
                        res.foot.length > 0 && end > res.head.length + filelen
                      )
                    });
                  };
                  res.writeHead(206, statusCodes[206], rhd);
        ...
        let useDeflate =
          ext != "zip" && filelen > 256 && acceptEncoding.match(/\bdeflate\b/);
        let useGzip =
          ...
          if (
            ...
            getCustomDirListingHeader(() => {
              getCustomDirListingFooter(() => {
                ...
                let htmlHead = `${
                  (!config.enableDirectoryListingWithDefaultHead ||
                  res.head == ""
                    ? !headerHasHTMLTag
  ...
  const directoryListingAndStaticFileServe = (fd) => {
    const operationsAfterFd = (fd) => {
      fs.fstat(fd, (err, stats) => {
        ...
        if (!dirImagesMissing) {
          if (stats.isDirectory()) {
            const indexes = ["html", "htm", "xhtml"];

            const checkForIndexes = (_id) => {
              if (!_id) _id = 0;
              if (_id >= indexes.length) {
                properDirectoryListingAndStaticFileServe(fd, stats);
                return;
              }
              fs.open(
                (readFrom + "/index." + indexes[_id]).replace(/\/+/g, "/"),
                "r",
                (err, newFd) => {
                  if (err) {
                    checkForIndexes(_id + 1);
                  } else {
                    fs.fstat(newFd, (err, stats) => {
                      if (err || !stats.isFile()) {
                        fs.close(newFd, () => {
                          checkForIndexes(_id + 1);
                        });
          ...
          if (stats.isFile()) {
            ...
            fs.close(fd, () => {
              res.error(404); // Pretend that the directory image doesn't exist.
              logFacilities.errmessage("Resource not found.");
            });
          }
        }
        ...
        if (err) {
          if (err.code == "ENOENT") {
            if (
              ...
              directoryListingAndStaticFileServe();
              return;
            } else {
              res.error(404);
              logFacilities.errmessage("Resource not found.");
